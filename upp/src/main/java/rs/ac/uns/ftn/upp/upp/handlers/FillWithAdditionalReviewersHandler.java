...
@Service
public class FillWithAdditionalReviewersHandler implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		System.err.println("Usli u FillWithAdditionalReviewersHandler");
			...
			if (!reviewers.isEmpty()) {
				...
				for(Customer r: reviewers) {
					if(!r.getReviews().stream().anyMatch(paper.getReviews()::contains)) {
						System.err.println("postoje recenzenti koji nisu recenzirali: " + r.getUsername());
		...
		System.err.println("izasli iz FillWithAdditionalReviewersHandler");
